...
begin
  ...
  if pg_catalog.coalesce(pg_catalog.cardinality(v_folders), 0) <> 3 then
    return false;
  end if;
...
declare
  ...
  v_extension text := pg_catalog.lower(
    pg_catalog.coalesce(pg_catalog.substring(v_filename, '\.([^.]+)$'), '')
  );
  v_authenticated boolean := v_user_id is not null;
  v_is_permanent boolean := false;
  v_path_valid boolean := pg_catalog.coalesce(pg_catalog.cardinality(v_folders), 0) = 3;
...
with check (
  ...
  and public.can_upload_idol_image_object(name)
);
